...
<template>
  ...
  <div class="pt-page pt-page-current pt-page-controller search-pager">
    <PageHeader ref="pageHeader" :labels="labels" pid="same001" version="1.0.0" showLanguage="true" @language-changed="changeLanguage" :multiLanguages="multiLanguages" :build="buildVersion" :visible="displayPageHeader" />
...
import { getLabelModel, getMultiLanguagesModel, getPermitModel, getMetaInfo } from "@willsofts/will-app";
...
export default {
    ...
    this.$nextTick(async () => {
      ...
      startApplication("same001",(data) => {
        if(data.type=="language") {
          ...
          loadAndMergeLabel("same001", (success) => {
            if (success) {
  ...
  methods: {
    ...
    loadDataCategories(loading,callback) {
      ...
      if(!loading) return;
      let jsondata = { names: ["tkprogtype","tkactive"] };
    ...
    resetDataCategories(lang) {
      ...
      let tkprogtype;
      let tkactive;
      let progtypedata = this.dataChunk["tkprogtype"];
      if(progtypedata) {
        tkprogtype = progtypedata.map((item) => { return { id: item.typeid, text: "EN"==lang?item.nameen:item.nameth } });
      }
      let activedata = this.dataChunk["tkactive"];
      if(activedata) {
        tkactive = activedata.map((item) => { return { id: item.typeid, text: "EN"==lang?item.nameen:item.nameth } });
      }
      console.log("tkprogtype",tkprogtype);
      console.log("tkactive",tkactive);
